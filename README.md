...
- 主页双栏：文件夹卡片墙（每个子文件夹单独成框）+ 最新资料更新列表，登录后直接进入
- 浏览模式：进入文件夹后左侧目录树（懒加载真实目录）+ 右侧文件区
- 最近更新：主页右侧按修改时间倒序列出全库最新文件，点击直达所在文件夹
...
2. 登录后进入主页：左侧是文件夹卡片（每个子文件夹单独成框），右侧是最新资料更新；点卡片进入该文件夹，点更新条目跳到所在文件夹。
3. 文件夹页：左侧目录树可展开子目录（按需加载），点文件夹名称切换右侧内容，面包屑可返回主页。
4. 上传：在上方面板选择目标文件夹（或点"＋ 新建文件夹"先建目录），再把文件拖入上传区或点击选择；同名文件会询问覆盖 / 重命名 / 取消。
5. 下载：点文件名或"下载"按钮；图片和 PDF 还有"预览"按钮。
6. 搜索：顶栏输入文件名，从下拉结果点击即可跳到所在文件夹。
7. 用完点右上角"退出登录"；刷新或关闭页面也会自动清除密码，下次需重新登录。
